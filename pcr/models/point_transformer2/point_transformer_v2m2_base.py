...
class KANLinear(nn.Module):
    def __init__(self, in_features, out_features, kernel_size=3):
        ...
        self.weights = nn.Parameter(torch.randn(out_features, in_features))  # Kernel weights
        self.bias = nn.Parameter(torch.zeros(out_features))  # Bias term

    def forward(self, x):
        # Apply kernelized transformation
        x = F.conv1d(x.unsqueeze(1), self.weights.unsqueeze(0), stride=1, padding=self.kernel_size//2)  # Applying convolution as kernel operation
        return F.relu(x.squeeze(1) + self.bias)
...
class GroupedVectorAttention(nn.Module):
    def __init__(
        ...
        # Replacing δmul and δbias with KAN
        if self.pe_multiplier:
