...
class KANLinear(nn.Module):
    def __init__(self, in_features, out_features, kernel_size=3):
        ...
        self.weights = nn.Parameter(torch.randn(out_features, in_features, kernel_size))  # 修正 conv1d 权重形状 [out_C, in_C, kernel_size]
        self.bias = nn.Parameter(torch.zeros(out_features))  # Bias term

    def forward(self, x):
        # 适配点云位置编码维度: [N, 1, K, 3] → [N, 3, K]
        if x.dim() == 4:
            x = x.squeeze(1).permute(0, 2, 1)  # [N, 1, K, 3] → [N, K, 3] → [N, 3, K]
        elif x.dim() == 3:
            x = x.permute(0, 2, 1)  # [N, K, C] → [N, C, K]
        elif x.dim() == 2:
            x = x.unsqueeze(2)  # [N, C] → [N, C, 1]

        # 执行 1D 卷积
        x = F.conv1d(x, self.weights, stride=1, padding=self.kernel_size//2)
        x = x.permute(0, 2, 1)  # [N, out_C, K] → [N, K, out_C]
        if x.size(1) == 1:
            x = x.squeeze(1)  # 如果 K=1，压回 [N, out_C]
...
class GroupedVectorAttention(nn.Module):
    def __init__(self,
        ...
        if self.pe_multiplier:
            self.linear_p_multiplier = KANLinear(3, embed_channels)
            
        if self.pe_bias:
            self.linear_p_bias = KANLinear(3, embed_channels)
